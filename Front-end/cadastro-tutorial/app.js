...
// Caso os dados já estejam no Local Storage, caso contrário, carrega os dados iniciais
var db2 = JSON.parse(localStorage.getItem('db_categoria'));
if (!db2) {
    db2 = db_categorias_inicial
...
function insertDropdown( ) {
    ...
    let contato = '';
    let i = 0;
    for (i = 0; i < db2.categorias.length; i++) {
        contato = db2.categorias[i].nome;
        
        texto = texto + `<option>${contato}</option>`;
    }

    document.getElementById("inputCategoria").innerHTML = texto;
}
...
function updateContato(id, contato) {
    ...
    db.data[index].url = contato.url,
    db.data[index].tutorial = contato.tutorial,
